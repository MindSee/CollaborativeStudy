...
%    end
    ...
    if ~isempty(iMrk),
      varargcat{iMrk}.time= varargcat{iMrk}.time + sum(T(1:ii-1));
      varargout{iMrk}= mrk_mergeMarkers(varargout{iMrk}, varargcat{iMrk});
    end
    if ~isempty(iMrkBV),
      error('The following commented lines (sample position based) are not adapted to the new toolbox (time-based)!')  
      %{
